#!/usr/bin/python3
# s1_50.py Sun 30/Sep/2018. 9.56 HRS.
# Sync RPi clock wiwh IWS server clock
# 
#
...
def pub_data():
    ...
    if client.connected_flag:
        while len(message_queue)>0:
            ...
            client.publish("s1/s1Qin", m_out, 1)
    else:
...
def on_message(client, userdata, message):
    """ 4 MQTT:  s1WO1: WO Data.  s1DT: server clock.
    s1WOIni: WO Initiate.  s1WOEnd: WO End."""
...
client = mqtt.Client(client_id="s1py")
...
# ---- Suscription
client.subscribe("s1/s1DT",1)
client.subscribe("s1/s1WO1",1)
client.subscribe("s1/s1WOIni",1)
client.subscribe("s1/s1WOEnd",1)
